...
const LookupValueList = () => {
  ...
  const handleFetchLookupValue = async () => {
    try {
      if (isFirstLoad) {
        setIsLoadingState(true);
      }
      const { data } = await apiHandler.lookupValueHandler.list(payload);
  ...
  const handleOnView = (item: any) => {
    navigate(`/lookup-value/create?action=view&id=${item?._id}`);
  };
  ...
  const handleOnDelete = async (item: any) => {
    try {
      ...
      const { data } = await apiHandler.lookupValueHandler.delete(item?._id);
      showToast('success', data?.message);
      handleFetchLookupValue();
    } catch (error: any) {
      showToast('error', error?.message);
    } finally {
  ...
  const handleOnEdit = (item: any) => {
    navigate(`/lookup-value/create?action=edit&id=${item?._id}`);
  };
  ...
  const TableCellData = [
    { value: 'Category', sort_field: 'category_id.name' },
    { value: 'Name', sort_field: 'name' },
    { value: 'Value Code', sort_field: 'value_code' },
    { value: 'Action' },
  ];

  return (
    <>
      <PageBreadcrumb pageTitle="Lookup Category Value" />
      <div className="space-y-6">
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
          <SearchBox
            payload={payload}
            setPayload={setPayload}
            onClick={() => navigate('/lookup-value/create')}
...
export default LookupValueList;
