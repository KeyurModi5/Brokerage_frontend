...
const LookupCategoryList = () => {
  ...
  const handleFetchLookupCategory = async () => {
    try {
      if (isFirstLoad) {
        setIsLoadingState(true);
      }
      const { data } = await apiHandler.lookupCategoryHandler.list(payload);
  ...
  const handleOnView = (item: any) => {
    navigate(`/lookup-category/create?action=view&id=${item?._id}`);
  };
  ...
  const handleOnDelete = async (item: any) => {
    try {
      ...
      const { data } = await apiHandler.lookupCategoryHandler.delete(item?._id);
      showToast('success', data?.message);
      handleFetchLookupCategory();
    } catch (error: any) {
      showToast('error', error?.message);
    } finally {
  ...
  const handleOnEdit = (item: any) => {
    navigate(`/lookup-category/create?action=edit&id=${item?._id}`);
  };
  ...
  const TableCellData = [
    ...
    { value: 'Category Code', sort_field: 'category_code' },
    { value: 'Action' },
  ];

  return (
    <>
      <PageBreadcrumb pageTitle="Lookup Category" />
      <div className="space-y-6">
        <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
          <SearchBox
            payload={payload}
            setPayload={setPayload}
            onClick={() => navigate('/lookup-category/create')}
...
export default LookupCategoryList;
