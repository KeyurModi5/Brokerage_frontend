...
const CityList = () => {
  ...
  const handleFetchCity = async () => {
    try {
      ...
      const { data } = await apiHandler.CityHandler.list(payload);
  ...
  const handleOnView = (item: any) => {
    navigate(`/city/create?action=view&id=${item?._id}`);
  };

  const handleOnDelete = async (item: any) => {
    try {
      dispatch(setIsLoading(true));
      const { data } = await apiHandler.CityHandler.delete(item?._id);
  ...
  const handleOnEdit = (item: any) => {
    navigate(`/city/create?action=edit&id=${item?._id}`);
  };
  ...
  const TableCellData = [
    { value: 'Country Name', sort_field: 'country_id.name' },
    { value: 'City Name', sort_field: 'name' },
    { value: 'Action' },
  ];
                ...
                <>
                  <Table>
                    ...
                    <TableHeader className="border-b border-gray-100 dark:border-white/[0.05]">
                      <TableRow className="xl:grid xl:grid-cols-3">
